...
export function recordToObject(record: Neo4jRecord): Record<string, unknown> {
  if (!("keys" in record) && "properties" in (record as unknown as Record<string, unknown>)) {
    return convertValue((record as unknown as { properties: unknown }).properties) as Record<string, unknown>;
  }
...
function convertValue(value: unknown): unknown {
  ...
  if (value && typeof value === "object") {
    ...
    if ("properties" in value)
      return convertValue((value as { properties: unknown }).properties);
