...
type Data=Record<"developers"|"projects"|"technologies"|"companies"|"skills"|"repositories"|"topics",string[]>;
const data=JSON.parse(await readFile(new URL("../data/seed-data.json",import.meta.url),"utf8")) as Data;
const id=(prefix:string,index:number)=>`${({d:"000000d1",p:"000000b1",t:"000000e1",c:"000000c1",s:"00000051",r:"000000f1",o:"00000001"} as Record<string,string>)[prefix]}-0000-4000-8000-${String(index+1).padStart(12,"0")}`;
const rows=(names:string[],prefix:string,extra:(name:string,i:number)=>Record<string,unknown>=()=>({}))=>names.map((name,i)=>({id:id(prefix,i),name,...extra(name,i)}));
...
try {
 ...
 const companies=rows(data.companies,"c",name=>({industry:"Technology"})); const skills=rows(data.skills,"s"); const repos=rows(data.repositories,"r",name=>({url:`https://github.com/graphsphere/${name}`})); const topics=rows(data.topics,"o");
 for(const [label,records] of [["Developer",devs],["Project",projects],["Technology",techs],["Company",companies],["Skill",skills],["Repository",repos],["Topic",topics]] as const) await session.run(`UNWIND $records AS record MERGE (n:${label} {id: record.id}) SET n += record`,{records});
 const passwordHash=await bcrypt.hash(env.DEMO_USER_PASSWORD,12); const demoId="u-0000-4000-8000-000000000001";
